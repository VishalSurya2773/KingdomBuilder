...
public class KingdomBuilderPanel extends JPanel implements MouseListener, ActionListener {
    ...
    public void drawBoard(Graphics g) {
        ...
        for (int i = 0; i < 4; i++) { // 620 x 528
            ...
            if (nums[i] == 1) {
                g.drawImage(b1, currX, currY, 400, 350, null);
                currX += 380;
            } else if (nums[i] == 2) {
                g.drawImage(b2, currX, currY, 400, 350, null);
                currX += 380;
            } else if (nums[i] == 3) {
                g.drawImage(b3, currX, currY, 400, 350, null);
                currX += 380;
            } else {
                g.drawImage(b7, currX, currY, 400, 350, null);
                currX += 380;
            }
            if (i == 1) {
                currY += 335;
